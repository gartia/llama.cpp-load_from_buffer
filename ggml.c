...
static bool gguf_bread_el(void *buffer, size_t buffer_size, void *dst, size_t size, size_t *offset) {
    if (*offset + size > buffer_size) return false;
    memcpy(dst, (char *)buffer + *offset, size);
    *offset += size;
    return true;
}
...
static bool gguf_bread_str(void *buffer, size_t buffer_size, struct gguf_str *p, size_t *offset) {
    p->n    = 0;
    p->data = NULL;

    bool ok = true;

    ok = ok && gguf_bread_el(buffer,buffer_size, &p->n, sizeof(p->n), offset);
    ...
    ok = ok && gguf_bread_el(buffer,buffer_size,  p->data, p->n, offset);
...
struct gguf_context * gguf_init_from_buffer(void*buffer, size_t buffer_size, struct gguf_init_params params)
{
    ...
    // check the magic before making allocations
    {
        gguf_bread_el(buffer, buffer_size, &magic, sizeof(magic), &offset);
        for (uint32_t i = 0; i < sizeof(magic); i++) {
            if (magic[i] != GGUF_MAGIC[i]) {
                fprintf(stderr, "%s: invalid magic characters '%c%c%c%c'\n", __func__, magic[0], magic[1], magic[2], magic[3]);
                return NULL;
    ...
    struct gguf_context * ctx = GGML_CALLOC(1, sizeof(struct gguf_context));
    {
        ...
        ok = ok && gguf_bread_el(buffer, buffer_size, &ctx->header.version, sizeof(ctx->header.version), &offset);
        ok = ok && gguf_bread_el(buffer, buffer_size, &ctx->header.n_tensors, sizeof(ctx->header.n_tensors), &offset);
        ok = ok && gguf_bread_el(buffer, buffer_size, &ctx->header.n_kv, sizeof(ctx->header.n_kv), &offset);
    ...
    // read the kv pairs
    {
        ...
        for (uint64_t i = 0; i < n_kv; ++i) {
            ...
            ok = ok && gguf_bread_str(buffer, buffer_size, &kv->key,                    &offset);
            ok = ok && gguf_bread_el (buffer, buffer_size, &kv->type, sizeof(kv->type), &offset);
            //fprintf(stderr, "%s: reading kv with key %s\n", __func__, kv->key.data);

            switch (kv->type) {
                case GGUF_TYPE_UINT8:   ok = ok && gguf_bread_el (buffer, buffer_size, &kv->value.uint8,   sizeof(kv->value.uint8),   &offset); break;
                case GGUF_TYPE_INT8:    ok = ok && gguf_bread_el (buffer, buffer_size, &kv->value.int8,    sizeof(kv->value.int8),    &offset); break;
                case GGUF_TYPE_UINT16:  ok = ok && gguf_bread_el (buffer, buffer_size, &kv->value.uint16,  sizeof(kv->value.uint16),  &offset); break;
                case GGUF_TYPE_INT16:   ok = ok && gguf_bread_el (buffer, buffer_size, &kv->value.int16,   sizeof(kv->value.int16),   &offset); break;
                case GGUF_TYPE_UINT32:  ok = ok && gguf_bread_el (buffer, buffer_size, &kv->value.uint32,  sizeof(kv->value.uint32),  &offset); break;
                case GGUF_TYPE_INT32:   ok = ok && gguf_bread_el (buffer, buffer_size, &kv->value.int32,   sizeof(kv->value.int32),   &offset); break;
                case GGUF_TYPE_FLOAT32: ok = ok && gguf_bread_el (buffer, buffer_size, &kv->value.float32, sizeof(kv->value.float32), &offset); break;
                case GGUF_TYPE_UINT64:  ok = ok && gguf_bread_el (buffer, buffer_size, &kv->value.uint64,  sizeof(kv->value.uint64),  &offset); break;
                case GGUF_TYPE_INT64:   ok = ok && gguf_bread_el (buffer, buffer_size, &kv->value.int64,   sizeof(kv->value.int64),   &offset); break;
                case GGUF_TYPE_FLOAT64: ok = ok && gguf_bread_el (buffer, buffer_size, &kv->value.float64, sizeof(kv->value.float64), &offset); break;
                case GGUF_TYPE_BOOL:    ok = ok && gguf_bread_el (buffer, buffer_size, &kv->value.bool_,   sizeof(kv->value.bool_),   &offset); break;
                case GGUF_TYPE_STRING:  ok = ok && gguf_bread_str(buffer, buffer_size, &kv->value.str,                                &offset); break;
                case GGUF_TYPE_ARRAY:
                    {
                        ok = ok && gguf_bread_el (buffer, buffer_size, &kv->value.arr.type, sizeof(kv->value.arr.type), &offset);
                        ok = ok && gguf_bread_el (buffer, buffer_size, &kv->value.arr.n,    sizeof(kv->value.arr.n),    &offset);
                            ...
                            case GGUF_TYPE_BOOL:
                                {
                                    ...
                                    ok = ok && gguf_bread_el (buffer, buffer_size, kv->value.arr.data, kv->value.arr.n * gguf_type_size(kv->value.arr.type), &offset);
                                } break;
                            case GGUF_TYPE_STRING:
                                {
                                    // prevent from integer overflow in the malloc below
                                    ...
                                    for (uint64_t j = 0; j < kv->value.arr.n; ++j) {
                                        ok = ok && gguf_bread_str(buffer,buffer_size, &((struct gguf_str *) kv->value.arr.data)[j], &offset);
                                    }
                                } break;
                            case GGUF_TYPE_ARRAY:
                            default: GGML_ASSERT(false && "invalid type"); break;
 ...
 if (ctx->header.n_tensors > 0) {
        ...
        for (uint64_t i = 0; i < ctx->header.n_tensors; ++i) {
            ...
            ok = ok && gguf_bread_str (buffer,buffer_size, &info->name,                          &offset);
            ok = ok && gguf_bread_el (buffer,buffer_size, &info->n_dims, sizeof(info->n_dims),  &offset);

            ok = ok && (info->n_dims <= GGML_MAX_DIMS);

            for (uint32_t j = 0; j < info->n_dims; ++j) {
                ok = ok && gguf_bread_el(buffer,buffer_size, &info->ne[j], sizeof(info->ne[j]), &offset);
            }

            ok = ok && gguf_bread_el (buffer,buffer_size, &info->type,   sizeof(info->type),    &offset);
            ok = ok && gguf_bread_el (buffer,buffer_size, &info->offset, sizeof(info->offset),  &offset);
    ...
    // we require the data section to be aligned, so take into account any padding
    {
        ...
        if (offset_pad != 0) {
            offset += ctx->alignment - offset_pad;//this needs to be changed
            //fseek(file, offset, SEEK_SET);
        }
    }
    ...
    if (params.ctx != NULL) {
        ...
        *params.ctx = ggml_init(pdata);
        ...
        if (!params.no_alloc) {
            ...
            // read the binary blob with the tensor data
            ok = ok && gguf_bread_el(buffer,buffer_size, data->data, ctx->size, &offset);
